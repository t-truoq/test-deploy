...
const MyBooking = () => {
  ...
  useEffect(() => {
    const params = Object.fromEntries(searchParams);
    const vnp_ResponseCode = params["vnp_ResponseCode"];
    const vnp_TxnRef = params["vnp_TxnRef"]; // Có thể dùng để lấy bookingId từ orderInfo

    if (vnp_ResponseCode === "00" && vnp_TxnRef) {
      // Giả định vnp_TxnRef chứa bookingId (cần xử lý để trích xuất bookingId từ orderInfo)
      const bookingId = parseInt(vnp_TxnRef.split("-")[1]); // Ví dụ: "Booking-76" -> 76
      const booking = bookings.find((b) => b.bookingId === bookingId);
      if (booking) {
        setSelectedBooking(booking);
        fetchBookingDetails(bookingId).then((details) => {
  ...
  const handlePayment = async () => {
    try {
      ...
      if (response.data && response.data.code === 0 && response.data.result) {
        ...
        window.location.href = response.data.result; // Chuyển hướng đến VNPay
      } else {
      ...
      if (error.response) {
        ...
        setErrorPopup("Failed to connect to payment server. Please try again.");
      }
    } finally {
      setIsPaying(false);
      // Không đóng popup ngay, để xử lý sau khi redirect
    }
  };
